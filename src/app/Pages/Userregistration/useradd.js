...
export default function UserAdd({ open, handleClose, handleAdd, dialogOpenCount, datasource, setDatasource, customerId }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const descriptionObj = { mode: data.mode, userId: operatorId, pageList: data.mode === ROLE_OPERATOR && (cleanCustomerId(customerId) === CUSTOMER_IDS.PMI || cleanCustomerId(customerId) === CUSTOMER_IDS?.OFFICE_USER) ? ["operator"] : data.pagelist, password: encryptedPassword};
      const payload = {
        email,
